...
//		cout << "fail to open!" << endl;
	...
	while (1)
	{
		...
		//	break;
		//}
		matImg = cv::imread(argv[2]);

		clock_t start1, end1;
		start1 = clock();

		ST_IMAGE_DATA stImg1(matImg.cols, matImg.rows, matImg.channels());
		stImg1.data = matImg.data;

		std::vector<ST_FACE_DATA> vctFaces;
		FCFacedet->FCFGetFaces(stImg1, vctFaces, 10);
		...
		{
			rectangle(matImg, cv::Rect(vctFaces[u].x_pos, vctFaces[u].y_pos, vctFaces[u].width, vctFaces[u].height), Scalar(255, 0, 0), 2);
			for (int k = 0; k < 5; k++)
			{
				circle(matImg, cv::Point(vctFaces[u].landmark[k * 2], vctFaces[u].landmark[k * 2 + 1]), 2, Scalar(255, 255, 0), 4);
			}
		}

		imshow("Results", matImg);
		waitKey(10);
